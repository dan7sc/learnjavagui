...
public class GScreen extends JComponent implements CronometerListener  {
  ...
  private String arquivoImagem5;
  private GImage controledImg;
  private GImage[][] waterImg;
  ...
  public GScreen(String dir) {
    ...
    for(int i = 0; i < 10; i++) {
      for(int j = 0; j < 10; j++) {
        waterImg[i][j] = new GImage(arquivoImagem5, i + 1, j + 1);
      }
    }
  ...
  private void makeGrid(Graphics g) {
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < 10; j++) {
        ...
        g.drawImage(waterImg[i][j].getImage(), waterImg[i][j].getX() * 50, waterImg[i][j].getY() * 50, this);
      }
    }
  }
